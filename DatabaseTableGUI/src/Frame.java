...
public class Frame extends JFrame {
    ...
    private void initPagination() {
        if(!refresh)
            totalData = (App.getStudentService().count());
        else
            totalData = App.getStudentService().countOne(id);
        ...
        if(!orderByFName && !orderById && !orderByLName && !orderByDepartment){
            if(!refresh)
                {
                    studentTableModel.setList(App.getStudentService().findAll(page, rowCountPerPage));}
           else{
                studentTableModel.setList(App.getStudentService().find(id));
           }
        }
        else{
            if(orderById) {
                studentTableModel.setList(App.getStudentService().OrderByID(page, rowCountPerPage));
            }else if(orderByFName){
                studentTableModel.setList(App.getStudentService().OrderByName(page, rowCountPerPage));
            }else if(orderByLName){
                studentTableModel.setList(App.getStudentService().OrderByLastName(page, rowCountPerPage));
            }else if(orderByDepartment){
                studentTableModel.setList(App.getStudentService().OrderByDepartment(page, rowCountPerPage));
            }
        ...
        jTableProduct.setModel(studentTableModel);


        service=App.getStudentService();
    ...
    private void jButtonAddActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if(!cancel) {
            ...
            studentTableModel.save(student);
            initPagination();
    ...
    private void jButtonSearchActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        studentTableModel.setList(App.getStudentService().findAll(page, rowCountPerPage));
        input=JOptionPane.showInputDialog("Write the id of the student you want!Leave blank to refresh the list");
    ...
    private void jButtonRemoveActionPerformed(java.awt.event.ActionEvent evt) {
        long id;
        String input;
        int total=(App.getStudentService().count());
        int data=total;

        studentTableModel.setList(App.getStudentService().findAll(page, rowCountPerPage));
        input=JOptionPane.showInputDialog("Write the ID of the student you want to remove!");
        if(input==null || input.equals("")) {
            input="1";
        }else{
            id=Integer.parseInt(input);
            if(studentTableModel.findById(id)){
                service.remove(id);
                initPagination();
                total=(App.getStudentService().count());
                if(data-1==total)
                    JOptionPane.showMessageDialog(null, "Student deleted!");
            }else{
                JOptionPane.showMessageDialog(null, "No student has this ID!");
